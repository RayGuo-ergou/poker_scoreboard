<template>
  <v-container>
    ...
    <div v-if="inGame">
      <v-row>
        <v-col v-for="play in players" :key="play" cols="8" lg="6" md="6" xl="3"
          ><Score @result="result" ref="scoreboard" :username="play"></Score>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="6">
...
export default {
  ...
  data: () => ({
    ...
    players: $cookies.isKey('names') ? JSON.parse($cookies.get('names')) : [],
    finalResult: [],
    resultSeen: false,
  }),
